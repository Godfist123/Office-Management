...
<head>
							   ...
							   <table id="zero_config" class="table  table-bordered table-hover">
								   ...
								   <tbody class="customtable">
										<c:forEach items="${pages.list }" var="employee" varStatus="vs">
											...
											<td>
												<a href="javascript:;" onclick="deleteRow(${employee.empId })">删除</a>
												<a href="employee/edit?empId=${employee.empId }">修改</a>
												
												<c:forEach items="${listLeader}" var="leader">
													<!-- 判断当前登录员工是否为经理,经理有修改状态权限 -->
													<c:if test="${emp.empId==leader.empId }">
														<!-- 判断当前记录的员工是否为经理,经理的权限不能被修改 -->
														<c:if test="${employee.empId!=leader.empId }">
															<div style="margin-top: 30px">
																<a href="employee/updateState?empId=${employee.empId }" class="btn btn-success" style="background-color:#609e9b;">${employee.state==1?'加锁':'启用' }</a>
															</div>
														</c:if>
													</c:if>
												</c:forEach>
								   			...
								   			<div style="float: right;">
								   				<ul class="pagination">
													<li class="page-item active"><a class="page-link" href="#">当前页/总页数: ${pages.currPage }/${pages.pages }</a></li>
								   					<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employee/list?currPage=${pages.first }&empName=${employee.empName }&loginName=${employee.loginName }&position=${employee.position }&departId=${employee.departId }&state=${employee.state}">首页</a></li>
								   					<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employee/list?currPage=${pages.pre }&empName=${employee.empName }&loginName=${employee.loginName }&position=${employee.position }&departId=${employee.departId }&state=${employee.state}">上一页</a></li>
								   					<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employee/list?currPage=${pages.next }&empName=${employee.empName }&loginName=${employee.loginName }&position=${employee.position }&departId=${employee.departId }&state=${employee.state}">下一页</a></li>
													<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employee/list?currPage=${pages.last }&empName=${employee.empName }&loginName=${employee.loginName }&position=${employee.position }&departId=${employee.departId }&state=${employee.state}">末页</a></li>
								   				</ul>
								   			</div>
								   		</td>
								   	</tr>
	...
	<script>
		...
		$("select[name=position]").val("${employee.position}");
		$("select[name=state]").val("${employee.state}");
